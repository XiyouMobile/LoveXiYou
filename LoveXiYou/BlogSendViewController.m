...
@interface BlogSendViewController (Private)
...
@implementation BlogSendViewController
...
- (id)initWithAppKey:(NSString *)appKey appSecret:(NSString *)appSecret text:(NSString *)text
{
    if (self = [super initWithFrame:CGRectMake(0, 0, 320, 480)])
    {
        ...
        if (![sendName isEqualToString:@"text"]) {
            NSString *textStr = [text stringByAppendingString:@"说点什么吧..."];
            NSLog(@"%@",textStr);
            [contentTextView setText:[text stringByAppendingFormat:@"说点什么吧..."]];
        }else {
            [contentTextView setText:@"说点什么吧..."];
        }
...
- (void)onSendButtonTouched:(id)sender
{
    if ([contentTextView.text isEqualToString:@""])
    {
        ...
        //评论
        NSDictionary *myDict = [[NSDictionary alloc] initWithObjectsAndKeys:contentTextView.text,@"status", nil];
        
        [engine sendWeiBoWithText:contentTextView.text image:nil];
        [myDict release]; 
        myDict = nil;
    }else if (tag == 103) {
        //评论
        NSDictionary *myDict = [[NSDictionary alloc] initWithObjectsAndKeys:[NSString stringWithFormat:@"%@",weibo_ID],@"id",contentTextView.text,@"comment", nil];
        
        [engine loadRequestWithMethodName:@"comments/create.json" httpMethod:@"POST" params:myDict postDataType:kWBRequestPostDataTypeNormal httpHeaderFields:nil];
        [myDict release]; 
        myDict = nil;
    }else if(tag == 104){
        //转发
        contentTextView.text = [NSString stringWithFormat:@"%@",self.blogContext];
        NSDictionary *myDict = [[NSDictionary alloc] initWithObjectsAndKeys:[NSString stringWithFormat:@"%@",weibo_ID],@"id",[NSString stringWithFormat:@"[转发]%@",self.blogContext],@"status", nil];
