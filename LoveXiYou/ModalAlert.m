...
+ (NSUInteger) ask: (NSString *) question message:(NSString *)message withCancel: (NSString *) cancelButtonTitle withButtons: (NSArray *) buttons{

    CFRunLoopRef currentLoop = CFRunLoopGetCurrent();
	
	// Create Alert
	ModalAlertDelegate *madelegate = [[ModalAlertDelegate alloc] initWithRunLoop:currentLoop];
	UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:question message:message delegate:madelegate cancelButtonTitle:cancelButtonTitle otherButtonTitles:nil];
...
+ (BOOL) ask: (id)formatstring,...
{
	...
	id statement = [[NSString alloc] initWithFormat:formatstring arguments:arglist];
	va_end(arglist);
	BOOL answer = ([ModalAlert ask:statement withCancel:nil withButtons:[NSArray arrayWithObjects:@"确定", @"取消", nil]] == 0);
	[statement release];
	return answer;
}
